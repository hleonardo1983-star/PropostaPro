...
function BlockedButton({ label, font }: { label: string; font: string }) {
  return (
    <Link
      href="/dashboard/plans"
      title="Período de avaliação encerrado — assine para continuar"
      style={{ background: '#e5e7eb', color: '#9ca3af', padding: '0.7rem 1.5rem', borderRadius: 100, textDecoration: 'none', fontSize: '0.9rem', fontWeight: 700, cursor: 'not-allowed', display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}
    >
      🔒 {label}
    </Link>
  )
  ...
  return (
    <div style={{ fontFamily: font }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        ...
        {isBlocked
          ? <BlockedButton label="Nova proposta" font={font} />
      ...
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(13,17,23,0.08)', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        {loading ? (
          ...
          <div style={{ padding: '4rem', textAlign: 'center', color: '#6b7280' }}>
            ...
            {!isBlocked && <Link href="/dashboard/proposals/new" style={{ color: '#2563eb', fontWeight: 700, textDecoration: 'none' }}>Criar primeira proposta →</Link>}
          </div>
        ) : (

    ...
    <div style={{ fontFamily: font }}>
      ...
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(13,17,23,0.08)', padding: '1.75rem', marginBottom: '1.5rem', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem', marginBottom: '1.5rem' }}>
          ...
          {trialDaysLeft !== null && (
            <div style={{
              background: trialDaysLeft === 0 ? 'rgba(220,38,38,0.08)' : trialDaysLeft <= 3 ? 'rgba(37,99,235,0.08)' : 'rgba(5,150,105,0.08)',
              border: `1px solid ${trialDaysLeft === 0 ? 'rgba(220,38,38,0.25)' : trialDaysLeft <= 3 ? 'rgba(37,99,235,0.2)' : 'rgba(5,150,105,0.2)'}`,
              borderRadius: 12, padding: '0.85rem 1.25rem', textAlign: 'center'
            }}>
              <p style={{ fontSize: '1.5rem', fontWeight: 800, color: trialDaysLeft === 0 ? '#dc2626' : trialDaysLeft <= 3 ? '#2563eb' : '#065f46', lineHeight: 1 }}>{trialDaysLeft > 0 ? trialDaysLeft : '🔒'}</p>
              <p style={{ fontSize: '0.75rem', fontWeight: 600, color: trialDaysLeft === 0 ? '#dc2626' : trialDaysLeft <= 3 ? '#2563eb' : '#065f46' }}>
                {trialDaysLeft > 0 ? 'dias de trial' : 'Trial encerrado'}
              </p>
            </div>
          )}
        </div>
      ...
      <div style={{ background: 'rgba(29,78,216,0.06)', border: '1px solid rgba(29,78,216,0.15)', borderRadius: 12, padding: '1.25rem 1.5rem', marginTop: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
        ...
        <div>
          ...
          <p style={{ color: '#6b7280', fontSize: '0.82rem' }}>A integração com Asaas está sendo implementada. Em breve você poderá assinar diretamente pelo sistema. Entre em contato conosco para liberar seu acesso antecipado.</p>
        </div>
      </div>
    </div>
  )

...
export default function PublicProposalPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase.rpc('get_public_proposal', { p_token: token })
      if (!data) { setLoading(false); return }
      const p = { ...data.proposal, clients: data.client }
      setProposal(p)
      if (p.status === 'signed') setSigned(true)
      setItems(data.items || [])
      setTenant(data.tenant)
      if (p.status === 'sent') {
        await supabase.rpc('mark_public_proposal_viewed', { p_token: token })
      }
      setLoading(false)
    }
    load()
  ...
  function renderItemSection({ list, title, subtotal, icon }: { list: any[]; title: string; subtotal: number; icon: string }) {
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: '#f8f7f4', fontFamily: font }}>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem' }}>
          ...
          {/* Quadro de Serviços */}
          {renderItemSection({ list: services, title: 'Serviços', subtotal: totalServices, icon: '🛠️' })}

          {/* Quadro de Produtos */}
          {renderItemSection({ list: products, title: 'Produtos', subtotal: totalProducts, icon: '📦' })}

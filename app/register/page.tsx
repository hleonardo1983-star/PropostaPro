...
export default function RegisterPage() {
  ...
  async function handleRegister(e: React.FormEvent) {
    ...
    try {
      ...
      const { data: tenant, error: tenantError } = await supabase
        .from('tenants').insert({ name: form.company, slug }).select().single()
      if (tenantError) throw tenantError

      const { error: profileError } = await supabase
        .from('profiles').insert({ id: authData.user.id, tenant_id: tenant.id, full_name: form.name, role: 'owner' })
  ...
  const inputStyle = {width:"100%",padding:"0.75rem 1rem",borderRadius:10,border:"1.5px solid rgba(15,14,12,0.15)",fontSize:"0.95rem",outline:"none",background:"#f5f0e8"}
          ...
          {[
            ...
            <div key={f.key}>
              ...
              <input type={f.type} value={(form as any)[f.key]} onChange={set(f.key)} required
                style={inputStyle} placeholder={f.placeholder} minLength={f.key==="password"?8:undefined} />
            </div>
          ))}

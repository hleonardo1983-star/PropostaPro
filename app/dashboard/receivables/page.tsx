...
export default function ReceivablesPage() {
  ...
  const inputStyle: React.CSSProperties = {
    ...
    border: '1.5px solid rgba(17,24,39,0.15)', fontSize: '0.875rem',
  ...
  return (
    <div style={{ fontFamily: font }}>
          ...
          <div style={{ background: 'white', borderRadius: 20, padding: '2.5rem', width: '100%', maxWidth: 420, boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', marginTop: '2rem', justifyContent: 'flex-end' }}>
              <button onClick={() => setEditingId(null)}
                style={{ background: 'transparent', border: '1.5px solid rgba(17,24,39,0.15)', padding: '0.65rem 1.25rem', borderRadius: 100, cursor: 'pointer', fontSize: '0.875rem', fontWeight: 600, fontFamily: font }}>
                Cancelar
              </button>
              <button onClick={saveEdit} disabled={saving}
                style={{ background: '#0d1117', color: 'white', border: 'none', padding: '0.65rem 1.5rem', borderRadius: 100, cursor: 'pointer', fontSize: '0.875rem', fontWeight: 700, fontFamily: font, opacity: saving ? 0.7 : 1 }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '0.9rem', fontWeight: 500 }}>Ordenadas por data de vencimento (mais próximas primeiro)</p>
      </div>
                      ...
                      <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                        {r.status === 'pending' && (
                          <button onClick={() => markPaid(r.id)}
                            style={{ background: 'rgba(5,150,105,0.1)', color: '#065f46', border: 'none', padding: '0.35rem 0.8rem', borderRadius: 100, cursor: 'pointer', fontSize: '0.78rem', fontWeight: 700, fontFamily: font, whiteSpace: 'nowrap' }}>
                            Marcar pago
                          </button>
                        )}
                        <button onClick={() => openEdit(r)}
                          style={{ background: 'transparent', border: '1px solid rgba(13,17,23,0.15)', color: '#6b7280', padding: '0.35rem 0.8rem', borderRadius: 100, cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, fontFamily: font }}
                          onMouseEnter={e => { e.currentTarget.style.borderColor = '#0d1117'; e.currentTarget.style.color = '#0d1117' }}
                          onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(13,17,23,0.15)'; e.currentTarget.style.color = '#6b7280' }}>
                          Editar
                        </button>
                      </div>
                    </td>

...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const { data } = await supabase
        .from('profiles')
        .select('full_name, role, tenant_id, tenants(name, primary_color, logo_url)')
        .eq('id', user.id)
        .single()
      if (data) {
  ...
  async function uploadLogo(file: File) {
    ...
    try {
      ...
      setMsgCompany(`Erro: ${err.message}`)
    }
  ...
  return (
    <div style={{ maxWidth: 600, fontFamily: font }}>
      ...
      {/* Dados pessoais */}
      <div style={cardStyle}>
        <h2 style={{ fontWeight: 700, fontSize: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>👤 Dados pessoais</h2>
      ...
      {/* Dados da empresa */}
      <div style={cardStyle}>
        <h2 style={{ fontWeight: 700, fontSize: '1rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>🏢 Dados da empresa</h2>
        <form onSubmit={saveCompany} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
              ...
              {company.logo_url ? (
                <div style={{ position: 'relative' }}>
                  ...
                  <button type="button" onClick={removeLogo}
                    style={{ position: 'absolute', top: -8, right: -8, width: 22, height: 22, background: '#dc2626', color: 'white', border: 'none', borderRadius: '50%', cursor: 'pointer', fontSize: '0.7rem', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ✕
                  </button>
                </div>
              ) : (
                <div style={{ width: 80, height: 80, border: '2px dashed rgba(13,17,23,0.15)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb', color: '#9ca3af', fontSize: '1.75rem' }}>
                  🏢
                </div>
              )}
              <div>
                ...
                <button type="button" onClick={() => fileInputRef.current?.click()} disabled={uploadingLogo}
                  style={{ background: '#0d1117', color: 'white', border: 'none', padding: '0.6rem 1.25rem', borderRadius: 100, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 700, fontFamily: font, opacity: uploadingLogo ? 0.7 : 1, display: 'block', marginBottom: '0.4rem' }}>
          ...
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
              <input type="color" value={company.primary_color} onChange={e => setCompany(c => ({ ...c, primary_color: e.target.value }))}
                style={{ width: 48, height: 48, borderRadius: 10, border: '1.5px solid rgba(13,17,23,0.15)', cursor: 'pointer', padding: 2, background: 'white' }} />
              <div>
...
function ChangePassword({ supabase, font }: { supabase: any; font: string }) {
  ...
  return sent ? (
    <div style={{ background: 'rgba(5,150,105,0.08)', border: '1px solid rgba(5,150,105,0.2)', borderRadius: 10, padding: '0.85rem 1rem', color: '#065f46', fontSize: '0.875rem', fontWeight: 600 }}>

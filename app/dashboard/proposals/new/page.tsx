...
export default function NewProposalPage() {
  ...
  function addItem(type: 'service' | 'product') {
  ...
  function ItemBlock({ type, icon, label, list }: { type: 'service' | 'product'; icon: string; label: string; list: Item[] }) {
    const subtotal = list.reduce((s, i) => s + i.quantity * i.unit_price, 0)
    return (
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(13,17,23,0.08)', overflow: 'hidden', marginBottom: '1.5rem', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ background: '#0d1117', padding: '1rem 1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            <span style={{ background: 'rgba(255,255,255,0.15)', color: 'rgba(255,255,255,0.8)', fontSize: '0.72rem', fontWeight: 700, padding: '0.15rem 0.6rem', borderRadius: 100 }}>{list.length} item{list.length !== 1 ? 's' : ''}</span>
          </div>
          {list.length > 0 && (
                ...
                <div key={item.id} style={{ display: 'grid', gridTemplateColumns: '1fr 90px 150px 36px', gap: '0.75rem', alignItems: 'end' }}>
                  <div>
                    ...
                    <input value={item.description} onChange={e => updateItem(item.id, 'description', e.target.value)}
                      style={inputStyle} placeholder={type === 'service' ? 'Ex: Consultoria mensal' : 'Ex: Notebook Dell'} />
                  </div>
                  <div>
                    {idx === 0 && <label style={labelStyle}>Qtd</label>}
                    <input type="number" value={item.quantity} min={1} step={0.01}
                      onChange={e => updateItem(item.id, 'quantity', parseFloat(e.target.value) || 1)} style={inputStyle} />
                  </div>
                  <div>
                    {idx === 0 && <label style={labelStyle}>Valor unitário</label>}
                    <input value={item.unit_price_display}
                      onChange={e => handlePriceInput(item.id, e.target.value)}
                      style={inputStyle} placeholder="R$ 0,00" inputMode="numeric" />
                  </div>
                  <div>
                    {idx === 0 && <label style={labelStyle}>&nbsp;</label>}
                    <button type="button" onClick={() => removeItem(item.id)} disabled={items.length === 1}
                      style={{ width: 36, height: 38, borderRadius: 8, border: '1px solid rgba(13,17,23,0.15)', background: 'white', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#9ca3af', fontSize: '0.9rem', fontFamily: font }}>
                      ✕
                    </button>
                  </div>
                </div>
  ...
  return (
    <div style={{ maxWidth: 780, fontFamily: font }}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        {/* Quadro Serviços */}
        <ItemBlock type="service" icon="🛠️" label="Serviços" list={services} />

        {/* Quadro Produtos */}
        <ItemBlock type="product" icon="📦" label="Produtos" list={products} />

      ...
      <div style={{ minHeight: '100vh', background: '#f8f7f4', fontFamily: font }}>
        <div className="no-print" style={{ background: 'white', borderBottom: '1px solid rgba(17,24,39,0.08)', padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <button onClick={handlePrint} style={{ background: '#0d1117', color: 'white', border: 'none', padding: '0.6rem 1.25rem', borderRadius: 100, cursor: 'pointer', fontSize: '0.85rem', fontWeight: 700, fontFamily: font }}>⬇️ Baixar PDF</button>
          </div>
        </div>
        ...
        <div style={{ maxWidth: 800, margin: '0 auto', padding: '2rem 1.5rem' }}>
          <div style={{ background: 'white', borderRadius: 16, padding: '2rem', marginBottom: '1.5rem', border: '1px solid rgba(13,17,23,0.08)', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: `3px solid ${accent}` }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ background: accent, color: 'white', padding: '0.4rem 1rem', borderRadius: 100, fontSize: '0.85rem', fontWeight: 700, marginBottom: '0.4rem', display: 'inline-block' }}>Proposta #{proposal.number}</div>
                <p style={{ fontSize: '0.8rem', color: '#6b7280', marginTop: '0.25rem' }}>{new Date(proposal.created_at).toLocaleDateString('pt-BR', { day: '2-digit', month: 'long', year: 'numeric' })}</p>
              </div>
            </div>
          ...
          {!signed ? (
            <div className="no-print" style={{ background: 'white', borderRadius: 16, border: '1.5px solid rgba(13,17,23,0.12)', padding: '2rem', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
              ...
              <input value={signerName} onChange={e => setSignerName(e.target.value)} style={{ width: '100%', padding: '0.75rem 1rem', borderRadius: 10, border: '1.5px solid rgba(13,17,23,0.15)', fontSize: '0.95rem', outline: 'none', marginBottom: '1rem', fontFamily: font }} placeholder="Digite seu nome completo" />
              <button onClick={handleSign} disabled={signing || !signerName.trim()} style={{ width: '100%', background: '#065f46', color: 'white', padding: '0.95rem', borderRadius: 100, border: 'none', fontWeight: 800, fontSize: '1rem', cursor: signing || !signerName.trim() ? 'not-allowed' : 'pointer', opacity: signing || !signerName.trim() ? 0.6 : 1, fontFamily: font }}>

      ...
      <div style={{ background: 'white', borderRadius: 16, border: '1px solid rgba(13,17,23,0.08)', padding: '1.75rem', marginBottom: '1.5rem', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <input
            ...
            type="email"
            disabled={activeUsers >= limit || isBlocked}
          />
          <button onClick={sendInvite} disabled={inviting || !inviteEmail.trim() || activeUsers >= limit || isBlocked}
            style={{ background: (activeUsers >= limit || isBlocked) ? '#d1d5db' : '#0d1117', color: 'white', border: 'none', padding: '0.7rem 1.5rem', borderRadius: 100, cursor: (activeUsers >= limit || isBlocked) ? 'not-allowed' : 'pointer', fontSize: '0.875rem', fontWeight: 700, fontFamily: font, whiteSpace: 'nowrap', opacity: inviting ? 0.7 : 1 }}>
            {isBlocked ? '🔒 Trial expirado' : inviting ? 'Enviando...' : 'Enviar convite'}
          </button>
        </div>
        {msg && (

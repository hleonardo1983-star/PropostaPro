...
export default function ClientsPage() {
  ...
  function openNew() { if (isBlocked) return; setEditClient(null); setForm({ name: '', email: '', phone: '', document: '', notes: '' }); setShowModal(true) }
  function openEdit(c: any) { if (isBlocked) return; setEditClient(c); setForm({ name: c.name || '', email: c.email || '', phone: c.phone || '', document: c.document || '', notes: c.notes || '' }); setShowModal(true) }
  ...
  return (
    <div style={{ fontFamily: font }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        ...
        <button onClick={openNew} disabled={isBlocked} style={{ background: isBlocked ? '#e5e7eb' : '#2563eb', color: isBlocked ? '#9ca3af' : 'white', padding: '0.7rem 1.5rem', borderRadius: 100, border: 'none', cursor: isBlocked ? 'not-allowed' : 'pointer', fontSize: '0.9rem', fontWeight: 700, fontFamily: font, display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          {isBlocked ? '🔒' : '+'} {isBlocked ? <Link href="/dashboard/plans" style={{ color: '#9ca3af', textDecoration: 'none' }}>Novo cliente</Link> : 'Novo cliente'}
        </button>
      </div>
                ...
                <tr key={c.id} style={{ borderBottom: '1px solid rgba(13,17,23,0.05)' }}
                  ...
                  <td style={{ padding: '1rem 1.25rem' }}>
                    <div style={{ display: 'flex', gap: '0.5rem' }}>
                      {!isBlocked && <button onClick={() => openEdit(c)} style={{ background: 'transparent', border: '1px solid rgba(13,17,23,0.12)', color: '#6b7280', padding: '0.3rem 0.75rem', borderRadius: 8, cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, fontFamily: font }}>Editar</button>}
                      {!isBlocked && <button onClick={() => setConfirmDelete(c)} style={{ background: 'transparent', border: '1px solid rgba(13,17,23,0.12)', color: '#9ca3af', padding: '0.3rem 0.75rem', borderRadius: 8, cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600, fontFamily: font }}
                      ...
                      {isBlocked && <span style={{ fontSize: '0.78rem', color: '#d1d5db', fontStyle: 'italic' }}>somente leitura</span>}
                    </div>
                  </td>
                </tr>
              ))}
